# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  fluxer-desktop = {
    pname = "fluxer-desktop";
    version = "2026.922.194050";
    src = fetchurl {
      url = "https://api.canary.fluxer.app/dl/desktop/canary/linux/x64/2026.922.194050/tar_gz";
      sha256 = "sha256-Dy6B22x3FvG67j8Q0ykI1dAajmJK3K7dhNPUaKyKfHY=";
    };
  };
  fluxer-desktop-darwin = {
    pname = "fluxer-desktop-darwin";
    version = "2026.922.194050";
    src = fetchurl {
      url = "https://api.canary.fluxer.app/dl/desktop/canary/darwin/arm64/2026.922.194050/zip";
      sha256 = "sha256-rsslVEC5Zyhk+mv7Ge4OKwLkb8UwIkv98d8c3Ur8G6Q=";
    };
  };
  hytale-launcher = {
    pname = "hytale-launcher";
    version = "2026.09.21-909ac0c";
    src = fetchurl {
      url = "https://launcher.hytale.com/builds/release/linux/amd64/hytale-launcher-2026.09.21-909ac0c.zip";
      sha256 = "sha256-WZFaVrkzuhNSQdJWF8J50PsaSbj4Vhldx/wcQ2j4tVs=";
    };
  };
  jellium-desktop = {
    pname = "jellium-desktop";
    version = "14dc0845981cb7c0aef5fa93ff539d927c8f0eb2";
    src = fetchgit {
      url = "https://github.com/andrewrabert/jellium-desktop.git";
      rev = "14dc0845981cb7c0aef5fa93ff539d927c8f0eb2";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-e4fanrXusIq3FsVTPqPzNRyye7ja6t6UnjX4mcjHPrk=";
    };
    date = "2026-09-19";
  };
  libdrm = {
    pname = "libdrm";
    version = "b97cbde15c5c3abfe44d78e8f57139e50f612fec";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/mesa/drm.git";
      rev = "b97cbde15c5c3abfe44d78e8f57139e50f612fec";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-6Pz1RSi5CZ5zjxO4c7TPsAYU8kOcbpBkTSIvGKb+H/M=";
    };
    date = "2026-09-17";
  };
  mesa = {
    pname = "mesa";
    version = "523ed9c203226d58a91efe1a392cef2f3aa8bc1c";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/mesa/mesa.git";
      rev = "523ed9c203226d58a91efe1a392cef2f3aa8bc1c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-vl23048g3cYveZHLIlBP3eb2aN4e1b4qDtrevKDZDdE=";
    };
    date = "2026-09-23";
  };
  modengine3 = {
    pname = "modengine3";
    version = "v0.13.0";
    src = fetchurl {
      url = "https://github.com/garyttierney/me3/releases/download/v0.13.0/me3-linux-amd64.tar.gz";
      sha256 = "sha256-8xV53uv3W0cCBZaUtmS92JF8R3OrDCZnFcqPkuHpP8E=";
    };
  };
  oodle-lib = {
    pname = "oodle-lib";
    version = "0.2.3";
    src = fetchurl {
      url = "https://github.com/new-world-tools/go-oodle/releases/download/v0.2.3-files/liboo2corelinux64.so.9";
      sha256 = "sha256-c1RlXrJbWH3DTL+YaWuR4w5tej8O7604cubBt274am4=";
    };
  };
  opengoal-launcher = {
    pname = "opengoal-launcher";
    version = "2.11.1";
    src = fetchurl {
      url = "https://github.com/open-goal/launcher/releases/download/v2.11.1/OpenGOAL-Launcher_2.11.1_amd64.AppImage";
      sha256 = "sha256-e43pKfAurZoCxMHCD0HlY/odZfQS9fZjSxPjPpRJBak=";
    };
  };
  pokemmo = {
    pname = "pokemmo";
    version = "271486206";
    src = fetchurl {
      url = "https://pokemmo.com/download_file/1/";
      sha256 = "sha256-eFJBIDbpawkRL2Zr3HE41+YY0bqvVCUqzQr0cdj2xM4=";
    };
  };
  proton-cachyos = {
    pname = "proton-cachyos";
    version = "cachyos-11.0-20260703-slr";
    src = fetchurl {
      url = "https://github.com/CachyOS/proton-cachyos/releases/download/cachyos-11.0-20260703-slr/proton-cachyos-11.0-20260703-slr-x86_64.tar.xz";
      sha256 = "sha256-Yv9LJ1AYByPMAFOGCP5ofiHR2Rox72TOGnyfRsPbMQs=";
    };
  };
  proton-cachyos-x86_64-v3 = {
    pname = "proton-cachyos-x86_64-v3";
    version = "cachyos-11.0-20260703-slr";
    src = fetchurl {
      url = "https://github.com/CachyOS/proton-cachyos/releases/download/cachyos-11.0-20260703-slr/proton-cachyos-11.0-20260703-slr-x86_64_v3.tar.xz";
      sha256 = "sha256-A+zUK9fUdOm6RDzoly2WeKH6Osvykg12HzU5eUbs4oQ=";
    };
  };
  pseudoregalia-rando = {
    pname = "pseudoregalia-rando";
    version = "1.8.6";
    src = fetchurl {
      url = "https://github.com/pseudoregalia-modding/rando/archive/1.8.6.tar.gz";
      sha256 = "sha256-viI3nxgwdTxH3m4dG564Lay45Q2m1AUlelYsABAdt/Q=";
    };
  };
  vencord = {
    pname = "vencord";
    version = "59a54286542651fff5ea53f0ce6cadf2a6aa7521";
    src = fetchgit {
      url = "https://github.com/Vendicated/Vencord.git";
      rev = "59a54286542651fff5ea53f0ce6cadf2a6aa7521";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-j3gCRDBD64o4W+4a71ADCtP0BM8imbfyQV+uvEUtk2A=";
    };
    date = "2026-09-15";
  };
  vintagestory = {
    pname = "vintagestory";
    version = "stable/vs_client_linux-x64_1.22.7";
    src = fetchurl {
      url = "https://cdn.vintagestory.at/gamefiles/stable/vs_client_linux-x64_1.22.7.tar.gz";
      sha256 = "sha256-SDzGgnpQwIxcQczQJixvEiPBBOVMpxNcRmBSeNifXeE=";
    };
  };
  wayland = {
    pname = "wayland";
    version = "1.26.0";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/wayland/wayland.git";
      rev = "1.26.0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dnaXYcVfc4DfNRsAEL316eSTen9tpf+nuQbActege4E=";
    };
  };
  wayland-protocols = {
    pname = "wayland-protocols";
    version = "819004adb3ab7e46f3fa3caef05b96e20434b244";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/wayland/wayland-protocols.git";
      rev = "819004adb3ab7e46f3fa3caef05b96e20434b244";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Fal+LAXzxouWmm+u8Dfi+G69eKsbTeMN+yMkv7/C9BQ=";
    };
    date = "2026-09-09";
  };
}
