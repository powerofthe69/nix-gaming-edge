# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  fluxer-desktop = {
    pname = "fluxer-desktop";
    version = "2026.903.231208";
    src = fetchurl {
      url = "https://api.canary.fluxer.app/dl/desktop/canary/linux/x64/latest/tar_gz";
      sha256 = "sha256-TetBDBc6ovZS/JahShiP8bw6pUe8YkV4tfZfXkb4kjs=";
    };
  };
  fluxer-desktop-darwin = {
    pname = "fluxer-desktop-darwin";
    version = "2026.903.231208";
    src = fetchurl {
      url = "https://api.canary.fluxer.app/dl/desktop/canary/darwin/arm64/latest/zip";
      sha256 = "sha256-45aHLDs1JrZ/10I7Dx5l/6scRQ+aGztAs+waRrmG+m8=";
    };
  };
  hytale-launcher = {
    pname = "hytale-launcher";
    version = "2026.08.28-3d62362";
    src = fetchurl {
      url = "https://launcher.hytale.com/builds/release/linux/amd64/hytale-launcher-2026.08.28-3d62362.zip";
      sha256 = "sha256-DLFvaRSfwilOkkdOz4rcnmsQTFQZvtIITmFBfhV67hg=";
    };
  };
  jellium-desktop = {
    pname = "jellium-desktop";
    version = "28f2cf16a1f1b819884dd6a72919ca55bdf9bd73";
    src = fetchgit {
      url = "https://github.com/andrewrabert/jellium-desktop.git";
      rev = "28f2cf16a1f1b819884dd6a72919ca55bdf9bd73";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-fMk5bZRMi6FgTGigu3/fYX9sj1HmKW5mp+Ipc+BO+tQ=";
    };
    date = "2026-08-09";
  };
  libdrm = {
    pname = "libdrm";
    version = "773536b1e5dde694dd743815528aff8bb2cf2cc3";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/mesa/drm.git";
      rev = "773536b1e5dde694dd743815528aff8bb2cf2cc3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-7XBMU3bNsIKOwDXjdfVqf0fU1Bji18GKrvy7Uho7PeM=";
    };
    date = "2026-08-13";
  };
  mesa = {
    pname = "mesa";
    version = "84cbbd2a2eef7f13368fc9a1db2c73b2afa41829";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/mesa/mesa.git";
      rev = "84cbbd2a2eef7f13368fc9a1db2c73b2afa41829";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dchzjA74AscH8cLQ18k5fj6I8soEW6AFZvyXDJLRh+E=";
    };
    date = "2026-09-04";
  };
  modengine3 = {
    pname = "modengine3";
    version = "v0.13.0";
    src = fetchurl {
      url = "https://github.com/garyttierney/me3/releases/download/v0.13.0/me3-linux-amd64.tar.gz";
      sha256 = "sha256-8xV53uv3W0cCBZaUtmS92JF8R3OrDCZnFcqPkuHpP8E=";
    };
  };
  oodle-lib = {
    pname = "oodle-lib";
    version = "0.2.3";
    src = fetchurl {
      url = "https://github.com/new-world-tools/go-oodle/releases/download/v0.2.3-files/liboo2corelinux64.so.9";
      sha256 = "sha256-c1RlXrJbWH3DTL+YaWuR4w5tej8O7604cubBt274am4=";
    };
  };
  opengoal-launcher = {
    pname = "opengoal-launcher";
    version = "2.10.4";
    src = fetchurl {
      url = "https://github.com/open-goal/launcher/releases/download/v2.10.4/OpenGOAL-Launcher_2.10.4_amd64.AppImage";
      sha256 = "sha256-eikzjdOYNdJM9FM3O/HuCFDzf97Fm1nhbRedDyKZ1/s=";
    };
  };
  pokemmo = {
    pname = "pokemmo";
    version = "271486206";
    src = fetchurl {
      url = "https://pokemmo.com/download_file/1/";
      sha256 = "sha256-eFJBIDbpawkRL2Zr3HE41+YY0bqvVCUqzQr0cdj2xM4=";
    };
  };
  proton-cachyos = {
    pname = "proton-cachyos";
    version = "cachyos-11.0-20260703-slr";
    src = fetchurl {
      url = "https://github.com/CachyOS/proton-cachyos/releases/download/cachyos-11.0-20260703-slr/proton-cachyos-11.0-20260703-slr-x86_64.tar.xz";
      sha256 = "sha256-Yv9LJ1AYByPMAFOGCP5ofiHR2Rox72TOGnyfRsPbMQs=";
    };
  };
  proton-cachyos-x86_64-v3 = {
    pname = "proton-cachyos-x86_64-v3";
    version = "cachyos-11.0-20260703-slr";
    src = fetchurl {
      url = "https://github.com/CachyOS/proton-cachyos/releases/download/cachyos-11.0-20260703-slr/proton-cachyos-11.0-20260703-slr-x86_64_v3.tar.xz";
      sha256 = "sha256-A+zUK9fUdOm6RDzoly2WeKH6Osvykg12HzU5eUbs4oQ=";
    };
  };
  pseudoregalia-rando = {
    pname = "pseudoregalia-rando";
    version = "1.8.6";
    src = fetchurl {
      url = "https://github.com/pseudoregalia-modding/rando/archive/1.8.6.tar.gz";
      sha256 = "sha256-viI3nxgwdTxH3m4dG564Lay45Q2m1AUlelYsABAdt/Q=";
    };
  };
  vencord = {
    pname = "vencord";
    version = "0e40e433d7aa9168f656aba733d01e761b7ca8ca";
    src = fetchgit {
      url = "https://github.com/Vendicated/Vencord.git";
      rev = "0e40e433d7aa9168f656aba733d01e761b7ca8ca";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-IoyxQuFrTlpwTqYgqsbeoLMuw8Hh7IJlvQi7ULdNAR0=";
    };
    date = "2026-08-30";
  };
  vintagestory = {
    pname = "vintagestory";
    version = "stable/vs_client_linux-x64_1.22.7";
    src = fetchurl {
      url = "https://cdn.vintagestory.at/gamefiles/stable/vs_client_linux-x64_1.22.7.tar.gz";
      sha256 = "sha256-SDzGgnpQwIxcQczQJixvEiPBBOVMpxNcRmBSeNifXeE=";
    };
  };
  wayland = {
    pname = "wayland";
    version = "1.26.0";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/wayland/wayland.git";
      rev = "1.26.0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dnaXYcVfc4DfNRsAEL316eSTen9tpf+nuQbActege4E=";
    };
  };
  wayland-protocols = {
    pname = "wayland-protocols";
    version = "afb614d5fcbd02d261a6ae91920aa91cf3915a8a";
    src = fetchgit {
      url = "https://gitlab.freedesktop.org/wayland/wayland-protocols.git";
      rev = "afb614d5fcbd02d261a6ae91920aa91cf3915a8a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-EgUVZoH2V9suDICELRgrjNi7NqCRu9LjxDSE3VNgSz0=";
    };
    date = "2026-06-23";
  };
}
